...
export default {
  ...
  setup() {
    ...
    onMounted(async () => {
      try {
        const apiKeyResponse = await axios.get('/api/v1/get-maptiler-api-key/');
        const apiKey = apiKeyResponse.data.maptiler_api_key;
        ...
        const soundsResponse = await axios.get('/api/v1/sounds/', {
          headers: {
